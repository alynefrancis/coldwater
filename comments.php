...
<div id="comments">
    ...
    if ( post_password_required() ) : ?>
        <p class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'afrancis' ); ?></p>
    </div><!-- #comments -->
    <?php return;
    endif;
    ?>
    ...
    <?php if ( have_comments() ) : ?>
        ...
        <ol class="commentlist">
            <?php
                /* See afrancis_comment() in inc/functions/comments.php for more.  */
                wp_list_comments( array( 'callback' => 'afrancis_comment' ) );
            ?>
        </ol>

        <?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
        <nav id="comment-nav-below">
            <h1 class="assistive-text"><?php _e( 'Comment navigation', 'afrancis' ); ?></h1>
            <div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'afrancis' ) ); ?></div>
            <div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'afrancis' ) ); ?></div>
        </nav>
    ...
    ?>
        <p class="nocomments"><?php _e( 'Comments are closed.', 'afrancis' ); ?></p>
    <?php endif; ?>
